...
class OpenAIClassifier:
    ...
    def generate(self, messages):
        ...
        agencies = [
            a.split(':')[0]
            for a in r.message.content.strip().split('\n')
        ]
    ...
    def annotate(self, article):
        ...
        messages = [
            {'role': 'system', 'content': 'You are a helpful assistant and an expert in the history of natural langauge processing.'},
...
class HuggingFaceClassifier:
    ...
    def format_prompt(self, article):
        return (
            (src, self.prompt_template.format(
                article=article, question=question).strip())
            for src, question in self.questions.items()
        )

    def annotate(self, article):
        out = {}
        for src, prompt in self.format_prompt(article):
